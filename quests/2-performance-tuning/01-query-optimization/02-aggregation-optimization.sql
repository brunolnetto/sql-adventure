...
INSERT INTO sales (
    product_id, quantity, sale_date, customer_id, region, discount_percent
) VALUES
...
SELECT
    ...
    AVG(s.quantity * p.price * (1 - s.discount_percent / 100))
        AS avg_sale_value,
    SUM(s.quantity * p.price * (1 - s.discount_percent / 100))
    / COUNT(*) AS calculated_avg
FROM sales AS s
INNER JOIN products AS p ON s.product_id = p.product_id
GROUP BY p.name;
...
SELECT
    ...
    SUM(s.quantity * p.price * (1 - s.discount_percent / 100))
    - SUM(s.quantity * p.cost) AS total_profit
FROM sales AS s
INNER JOIN products AS p ON s.product_id = p.product_id
INNER JOIN categories AS c ON p.category_id = c.category_id
GROUP BY c.name;
...
INNER JOIN categories AS c ON ss.category_id = c.category_id
GROUP BY c.name;
...
SELECT
    p.name,
    SUM(CASE WHEN s.region = 'North' THEN s.quantity * p.price ELSE 0 END)
        AS north_revenue,
    SUM(CASE WHEN s.region = 'South' THEN s.quantity * p.price ELSE 0 END)
        AS south_revenue,
    SUM(CASE WHEN s.region = 'East' THEN s.quantity * p.price ELSE 0 END)
        AS east_revenue,
    SUM(CASE WHEN s.region = 'West' THEN s.quantity * p.price ELSE 0 END)
        AS west_revenue,
    SUM(s.quantity * p.price) AS total_revenue
FROM sales AS s
INNER JOIN products AS p ON s.product_id = p.product_id
...
SELECT
    ...
    AVG(CASE WHEN s.discount_percent > 0 THEN s.discount_percent END)
        AS avg_discount
FROM sales AS s
INNER JOIN products AS p ON s.product_id = p.product_id
GROUP BY p.name;
