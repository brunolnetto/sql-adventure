...
-- Insert sample data
INSERT INTO customers (first_name, last_name, email, registration_date, total_spent) VALUES
...
SELECT p.name, p.category,
       (SELECT COUNT(*) FROM order_items oi 
        ...
        WHERE oi.product_id = p.product_id AND o.status = 'completed') as sales_count,
...
SELECT c.first_name, c.last_name, top_product.product_id
FROM customers c
CROSS JOIN LATERAL (
    SELECT oi.product_id 
    FROM order_items oi 
...
SELECT first_name, last_name, total_spent, avg_customer_spending
FROM customer_stats
