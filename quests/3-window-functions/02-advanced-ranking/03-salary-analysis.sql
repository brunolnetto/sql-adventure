...
SELECT 
    ...
    COUNT(*) as employee_count,
    ROUND(AVG(salary), 2) as avg_salary,
    ROUND(PERCENT_RANK() OVER (ORDER BY AVG(salary)) * 100, 2) as education_experience_percentile,
...
WITH salary_analysis AS (
    SELECT 
        ...
        ROUND(PERCENT_RANK() OVER (ORDER BY salary) * 100, 2) as company_percentile,
        -- Department percentile
        ROUND(PERCENT_RANK() OVER (PARTITION BY department ORDER BY salary) * 100, 2) as dept_percentile,
        -- Experience-based percentile
        ROUND(PERCENT_RANK() OVER (PARTITION BY years_experience ORDER BY salary) * 100, 2) as exp_percentile,
        -- Education-based percentile
        ROUND(PERCENT_RANK() OVER (PARTITION BY education_level ORDER BY salary) * 100, 2) as edu_percentile,
        -- Salary bands
        NTILE(4) OVER (ORDER BY salary) as salary_quartile,
...
SELECT 
    ...
    dept_quintile,
    CASE 
        WHEN company_percentile >= 80 AND dept_percentile >= 80 THEN 'High Performer'
        WHEN company_percentile >= 60 AND dept_percentile >= 60 THEN 'Above Average'
        WHEN company_percentile >= 40 AND dept_percentile >= 40 THEN 'Average'
        WHEN company_percentile >= 20 AND dept_percentile >= 20 THEN 'Below Average'
        ELSE 'Needs Review'
...
SELECT 
    ...
    ROUND(AVG(salary), 2) as avg_salary,
    ROUND(PERCENT_RANK() OVER (ORDER BY AVG(salary)) * 100, 2) as salary_percentile,
    ROUND(AVG(years_experience), 1) as avg_experience,
    ROUND(PERCENT_RANK() OVER (ORDER BY AVG(years_experience)) * 100, 2) as exp_percentile,
...
SELECT 
    ...
    ROUND(PERCENT_RANK() OVER (ORDER BY AVG(salary)) * 100, 2) as year_percentile,
