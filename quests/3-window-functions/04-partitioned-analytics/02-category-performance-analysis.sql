...
SELECT
    ...
    ROUND(
        (
            PERCENT_RANK() OVER (PARTITION BY category ORDER BY sales_amount)
        )::NUMERIC,
        3
    ) AS category_percentile,
    ROUND((PERCENT_RANK() OVER (ORDER BY sales_amount))::NUMERIC, 3)
        AS overall_percentile,
    NTILE(4)
        OVER (PARTITION BY category ORDER BY sales_amount DESC)
        AS category_quartile,
...
SELECT
    ...
    ROW_NUMBER()
        ...
        AS category_rank,
    ROUND(
        (sales_amount * 100.0 / SUM(sales_amount) OVER (PARTITION BY category)),
        2
