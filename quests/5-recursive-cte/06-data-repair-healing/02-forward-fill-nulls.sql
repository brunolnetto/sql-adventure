...
WITH RECURSIVE forward_fill AS (
    ...
    SELECT
        ...
        ff.row_num + 1
    FROM time_series_data AS tsd
    INNER JOIN forward_fill AS ff ON tsd.sensor_id = ff.sensor_id
    WHERE
        tsd.id > ff.id
        AND tsd.id = (
            SELECT MIN(id)
            FROM time_series_data AS ts3
            WHERE
                ts3.sensor_id = tsd.sensor_id
                AND ts3.id > ff.id
        )
)
...
FROM forward_fill AS ff
INNER JOIN time_series_data AS tsd ON ff.id = tsd.id
