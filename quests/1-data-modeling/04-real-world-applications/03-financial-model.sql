...
CREATE TABLE accounts (
    ...
    account_status VARCHAR(20) DEFAULT 'active' CHECK (
        account_status IN ('active', 'suspended', 'closed', 'frozen')
    ),
...
CREATE TABLE transactions (
    ...
    transaction_type_id INT NOT NULL REFERENCES transaction_types (
        transaction_type_id
    ),
    ...
    status VARCHAR(20) DEFAULT 'pending' CHECK (
        status IN ('pending', 'posted', 'failed', 'reversed', 'disputed')
    ),
...
CREATE TABLE loans (
    ...
    loan_status VARCHAR(20) DEFAULT 'active' CHECK (
        loan_status IN ('active', 'paid_off', 'defaulted', 'charged_off')
    ),
...
CREATE TABLE loan_payments (
    ...
    remaining_balance DECIMAL(12, 2) NOT NULL,
    payment_status VARCHAR(20) DEFAULT 'scheduled' CHECK (
        payment_status IN ('scheduled', 'paid', 'late', 'missed')
    ),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
(
    ...
    'Checking Account',
    'CHK',
    'Basic checking account with no minimum balance',
    0.00,
    0.00,
    0.0001,
    500.00,
...
(
    ...
    'Premium Checking',
    'PREM',
    'Premium checking with higher limits',
    2500.00,
    0.00,
    0.0050,
    1000.00,
...
LEFT JOIN
    accounts AS a
    ON c.customer_id = a.customer_id AND a.account_status = 'active'
LEFT JOIN
    loans AS l
    ON c.customer_id = l.customer_id AND l.loan_status = 'active'
...
SELECT
    ...
    ROUND(SUM(a.interest_earned) * 100.0 / SUM(a.current_balance), 2)
        AS effective_interest_rate
FROM accounts AS a
INNER JOIN account_types AS at ON a.account_type_id = at.account_type_id
...
SELECT
    ...
    CASE
        ...
        WHEN
            t.amount > 5000 AND c.risk_level = 'high'
            THEN 'High Risk Large Transaction'
        WHEN
            t.merchant_category IN ('Gambling', 'Adult Entertainment')
            THEN 'Suspicious Category'
        ELSE 'Normal'
    END AS risk_flag
...
SELECT
    ...
    ROUND(
        COUNT(CASE WHEN l.loan_status = 'defaulted' THEN 1 END)
        * 100.0
        / COUNT(*),
        2
    ) AS default_rate,
    ROUND(
        SUM(
            CASE
                WHEN l.loan_status = 'defaulted' THEN l.remaining_balance ELSE 0
            END
        ),
        2
    ) AS defaulted_amount
FROM loans AS l
INNER JOIN loan_types AS lt ON l.loan_type_id = lt.loan_type_id
...
SELECT
    ...
    ROUND(
        SUM(t.fee_amount)
        * 100.0
        / SUM(SUM(t.fee_amount))
            OVER (PARTITION BY DATE_TRUNC('month', t.transaction_date)),
        2
    ) AS fee_percentage
FROM transactions AS t
INNER JOIN
...
SELECT
    ...
    ROUND(
        SUM(t.amount)
        - LAG(SUM(t.amount))
            OVER (ORDER BY DATE_TRUNC('month', t.transaction_date)),
        2
    ) AS interest_growth
FROM transactions AS t
WHERE
...
WITH account_metrics AS (
    SELECT
        ...
        ROUND(AVG(a.current_balance * at.interest_rate), 2)
            AS expected_interest_income,
        ROUND(
            SUM(t.fee_amount) - AVG(a.current_balance * at.interest_rate), 2
        ) AS net_revenue
    FROM accounts AS a
    INNER JOIN customers AS c ON a.customer_id = c.customer_id
    INNER JOIN account_types AS at ON a.account_type_id = at.account_type_id
...
SELECT
    ...
    CASE
        ...
        WHEN
            l.remaining_balance > l.principal_amount * 0.9
            THEN 'Close Monitoring'
        ELSE 'Normal'
...
ORDER BY risk_assessment ASC, l.remaining_balance DESC;
