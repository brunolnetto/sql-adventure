...
CREATE TABLE patient_addresses (
    ...
    address_type VARCHAR(20) NOT NULL CHECK (
        address_type IN ('home', 'work', 'billing', 'mailing')
    ),
...
CREATE TABLE encounters (
    ...
    encounter_type VARCHAR(50) NOT NULL CHECK (
        encounter_type IN (
            'office_visit', 'emergency', 'inpatient', 'outpatient', 'telehealth'
        )
    ),
...
(
    ...
    'Prescribe blood pressure medication, schedule follow-up',
    'completed'
),
(
    2,
    ...
    'Annual checkup',
    'Healthy',
    'Continue current medications, annual blood work',
    'completed'
),
(
    3,
    ...
    'Prescribe antibiotics, rest and fluids',
    'completed'
);
...
(
    ...
    26.2,
    4,
    'Elevated temperature, increased heart rate'
);
...
SELECT
    ...
    ROUND(AVG(EXTRACT(YEAR FROM AGE(CURRENT_DATE, date_of_birth))), 1)
        AS avg_age,
...
WHERE
    ...
    AND e.encounter_date >= CURRENT_DATE - INTERVAL '30 days'
GROUP BY
    p.provider_id, p.first_name, p.last_name, p.specialty, d.department_name
...
WITH vital_trends AS (
    SELECT
        ...
        temperature_f,
        ROW_NUMBER()
            OVER (PARTITION BY patient_id ORDER BY vital_date)
            AS reading_number
    FROM vital_signs
    WHERE
...
SELECT
    ...
    CASE
        WHEN
            vt.blood_pressure_systolic >= 140
            OR vt.blood_pressure_diastolic >= 90
            THEN 'High'
        WHEN
            vt.blood_pressure_systolic < 90 OR vt.blood_pressure_diastolic < 60
            THEN 'Low'
        ELSE 'Normal'
    END AS bp_status,
    CASE
        ...
        ELSE 'Normal'
    END AS hr_status
FROM vital_trends AS vt
INNER JOIN patients AS p ON vt.patient_id = p.patient_id
...
SELECT
    ...
    CASE
        ...
        WHEN
            pr.end_date <= CURRENT_DATE + INTERVAL '7 days'
            THEN 'Expiring Soon'
        ELSE 'Active'
    END AS prescription_status
FROM prescriptions AS pr
INNER JOIN patients AS p ON pr.patient_id = p.patient_id
INNER JOIN medications AS m ON pr.medication_id = m.medication_id
...
SELECT
    ...
    ROUND(
        (
            pa.quantity_dispensed::DECIMAL
            / (pa.days_prescribed * pa.doses_per_day)
        )
        * 100,
        2
...
FROM prescription_analysis AS pa
INNER JOIN patients AS p ON pa.patient_id = p.patient_id
INNER JOIN medications AS m ON pa.medication_id = m.medication_id
...
SELECT
    ...
    CASE
        WHEN
            vs.blood_pressure_systolic >= 180
            OR vs.blood_pressure_diastolic >= 110
            THEN 'Stage 3 Hypertension'
        WHEN
            vs.blood_pressure_systolic >= 160
            OR vs.blood_pressure_diastolic >= 100
            THEN 'Stage 2 Hypertension'
        WHEN
            vs.blood_pressure_systolic >= 140
            OR vs.blood_pressure_diastolic >= 90
            THEN 'Stage 1 Hypertension'
        WHEN
            vs.blood_pressure_systolic >= 130
            OR vs.blood_pressure_diastolic >= 80
            THEN 'Elevated'
        ELSE 'Normal'
    ...
    CASE
        WHEN
            vs.blood_pressure_systolic >= 180
            OR vs.blood_pressure_diastolic >= 110
            THEN 'Immediate medical attention required'
        WHEN
            vs.blood_pressure_systolic >= 160
            OR vs.blood_pressure_diastolic >= 100
            THEN 'Schedule follow-up within 1 week'
        WHEN
            vs.blood_pressure_systolic >= 140
            OR vs.blood_pressure_diastolic >= 90
            THEN 'Monitor and lifestyle changes'
        ELSE 'Continue monitoring'
    END AS recommendation
FROM vital_signs AS vs
INNER JOIN patients AS p ON vs.patient_id = p.patient_id
WHERE
    vs.blood_pressure_systolic >= 130 OR vs.blood_pressure_diastolic >= 80
...
SELECT
    ...
    CASE
        WHEN
            ea.days_since_previous <= 30
            THEN 'Schedule follow-up within 1 week'
        WHEN
            ea.days_since_previous <= 90
            THEN 'Schedule follow-up within 2 weeks'
        ELSE 'Standard follow-up'
    END AS recommendation
FROM encounter_analysis AS ea
INNER JOIN patients AS p ON ea.patient_id = p.patient_id
WHERE
    ea.days_since_previous IS NOT null
