...
SELECT
    ...
    JSONB_BUILD_OBJECT(
        'current_weather', JSONB_BUILD_OBJECT(
            'temperature',
            (response_data -> 'data' -> 'current' ->> 'temperature')::DECIMAL(
                4, 1
            ),
            'humidity',
            (response_data -> 'data' -> 'current' ->> 'humidity')::INT,
            'description',
            response_data -> 'data' -> 'current' ->> 'description',
            'wind_speed',
            (response_data -> 'data' -> 'current' ->> 'wind_speed')::DECIMAL(
                4, 1
            )
        ),
        'forecast_summary', JSONB_BUILD_OBJECT(
            'days_count',
            (SELECT COUNT(*) FROM jsonb_array_elements(response_data -> 'data' -> 'forecast')),
            'avg_high', (
                SELECT AVG((day ->> 'high')::INT)
                FROM
                    JSONB_ARRAY_ELEMENTS(
                        response_data -> 'data' -> 'forecast'
                    ) AS day
            ),
            'avg_low', (
                SELECT AVG((day ->> 'low')::INT)
                FROM
                    JSONB_ARRAY_ELEMENTS(
                        response_data -> 'data' -> 'forecast'
                    ) AS day
        ...
        'location_info', JSONB_BUILD_OBJECT(
            ...
            'coordinates',
            response_data -> 'data' -> 'location' -> 'coordinates'
        )
    ...
    CASE
        WHEN api_response ->> 'status' = 'success'
            THEN JSONB_BUILD_OBJECT(
                'data_extracted', TRUE,
                'response_size', (SELECT COUNT(*) FROM jsonb_object_keys(api_response -> 'data')),
                'has_metadata',
...
SELECT
    ...
    COUNT(*) FILTER (WHERE response_data ->> 'status' = 'success')
        AS successful_requests,
    COUNT(*) FILTER (WHERE response_data ->> 'status' = 'error')
        AS failed_requests,
    JSONB_BUILD_OBJECT(
        'success_rate', ROUND(
            (
        ...
        'last_successful',
        MAX(response_time) FILTER (
            WHERE response_data ->> 'status' = 'success'
        ),
        'common_errors',
        JSONB_AGG(DISTINCT response_data -> 'error' ->> 'code') FILTER (
            WHERE response_data ->> 'status' = 'error'
        )
    ) AS api_metrics
FROM api_responses
...
SELECT
    ...
    COUNT(*) AS total_requests,
    COUNT(*) FILTER (WHERE api_response ->> 'status' = 'success')
        AS successful_requests,
    COUNT(*) FILTER (WHERE api_response ->> 'status' = 'error')
        AS failed_requests,
    JSONB_BUILD_OBJECT(
        'success_rate', ROUND(
            (
                COUNT(*) FILTER (
                    WHERE api_response ->> 'status' = 'success'
                )::DECIMAL
                / COUNT(*)
            )
            * 100,
        ...
        'processing_success_rate', ROUND(
            (
                COUNT(*) FILTER (WHERE processing_status = 'processed')::DECIMAL
                / COUNT(*)
            )
            * 100,
            2
